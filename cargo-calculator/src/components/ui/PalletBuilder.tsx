...
const presets: Array<{ id: ApartmentPreset; label: string; hint: string }> = [
  { id: 'oneRoom', label: '1 к.к.', hint: 'типовая однокомнатная' },
  { id: 'twoRoom', label: '2 к.к.', hint: 'двухкомнатная + шкафы' },
  { id: 'threeRoom', label: '3 к.к.', hint: 'много мебели и коробок' }
];
...
export function PalletBuilder() {
  ...
  return (
    <section className="mb-6 rounded-3xl bg-white/55 p-4 ring-1 ring-black/5">
      <div className="mb-4 flex items-center justify-between gap-3"><h2 className="text-lg font-semibold text-gray-800">🏠 Предметы и пресеты</h2><span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-black text-[#d35400]">{pallets.length} объектов</span></div>

      <div className="mb-4 grid grid-cols-3 gap-2">
        {presets.map((preset) => <button key={preset.id} onClick={() => applyApartmentPreset(preset.id)} className={`rounded-2xl border p-3 text-left transition ${activePreset === preset.id ? 'border-[#ff6b00] bg-orange-50 shadow-md' : 'border-gray-200 bg-white/70 hover:border-orange-200'}`}><span className="block text-lg font-black text-gray-950">{preset.label}</span><span className="block text-xs font-bold text-gray-500">{preset.hint}</span></button>)}
      </div>
      ...
      <div className="mb-4 rounded-2xl bg-slate-950 p-3 text-white">
        ...
        <div className="grid grid-cols-2 gap-2 xl:grid-cols-3">
          {CATALOG.map((item) => <button key={item.kind} onClick={() => addCatalogItem(item.kind)} className="rounded-xl bg-white/10 p-2 text-left transition hover:bg-white/20"><span className="block text-base font-black">{item.emoji} {item.name}</span><span className="block text-[11px] font-semibold text-slate-300">{item.description}</span></button>)}
        </div>
      </div>

      {selected && <div className="mb-4 rounded-2xl bg-orange-50 p-3 ring-1 ring-orange-200"><div className="mb-2 text-sm font-black text-gray-950">Выбран: {selected.name}</div><div className="grid grid-cols-3 gap-2"><button onClick={() => liftSelected(0.1)} className="rounded-xl bg-white px-2 py-2 text-xs font-black text-emerald-700 shadow-sm">↑ поднять</button><button onClick={() => liftSelected(-0.1)} className="rounded-xl bg-white px-2 py-2 text-xs font-black text-red-700 shadow-sm">↓ опустить</button><button onClick={rotateSelectedY} className="rounded-xl bg-white px-2 py-2 text-xs font-black text-blue-700 shadow-sm">R 90°</button></div><div className="mt-2 text-xs font-semibold text-gray-600">Drag в 3D — перемещение, gizmo — вращение/XYZ, высота и коллизии проверяются.</div></div>}
      ...
      <div className="mt-4 max-h-44 space-y-2 overflow-auto pr-1">
        {pallets.map((item, index) => <div key={item.id} className={`flex items-center justify-between gap-2 rounded-2xl p-3 text-sm ${selectedPalletId === item.id ? 'bg-orange-50 ring-2 ring-[#ff6b00]' : 'bg-white/70'}`}><button className="text-left" onClick={() => selectPallet(item.id)}><span className="block font-black text-gray-900">#{index + 1} {item.name}</span><span className="text-xs text-gray-500">{item.dimensions.length}×{item.dimensions.width}×{item.dimensions.height} м · {item.weight} кг</span></button><button className="rounded-xl bg-red-50 px-3 py-2 font-bold text-red-600 hover:bg-red-100" onClick={() => removePallet(item.id)}>Удалить</button></div>)}
      </div>
    </section>
  );
}
