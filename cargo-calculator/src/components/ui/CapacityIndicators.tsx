...
export function CapacityIndicators() {
  ...
  return (
    <div className="pointer-events-none absolute left-6 top-6 z-10 w-[min(470px,calc(100%-48px))] rounded-3xl border border-white/50 bg-white/80 p-4 shadow-glass backdrop-blur-glass">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div><div className="text-sm font-black text-gray-900">Загрузка кузова</div><div className="text-xs font-semibold text-gray-500">{vehicle.label} · выбрано: {selectedIndex >= 0 ? `#${selectedIndex + 1}` : 'нет'}</div></div>
        <div className="rounded-full bg-[#10131b] px-3 py-1 text-xs font-black text-white ring-2 ring-[#ff6b00]/60">Drag + XYZ</div>
      </div>
...
function Meter({ label, value, text }: { label: string; value: number; text?: string }) {
  ...
  return <div className="mb-2 last:mb-0"><div className="mb-1 flex justify-between text-xs font-bold text-gray-600"><span>{label}</span><span>{text ?? `${value}%`}</span></div><div className="h-2 overflow-hidden rounded-full bg-gray-200"><div className={`h-full rounded-full ${color}`} style={{ width: `${safe}%` }} /></div></div>;
}
