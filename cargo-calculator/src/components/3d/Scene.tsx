...
function CameraController() {
  ...
  const getDesired = () => {
    ...
    switch (cameraMode) {
      case 'inside':
        return {
          pos: new THREE.Vector3(-L / 2 + 0.6, H * 0.55 + 0.3, 0.15),
          target: new THREE.Vector3(L / 2 - 0.4, H * 0.35, 0),
          fov: 68
        };
      case 'cabin':
        return {
          pos: new THREE.Vector3(-L / 2 - 0.9, 1.45, 0.4),
          target: new THREE.Vector3(L / 2 * 0.2, 0.8, 0),
          fov: 62
        };
      case 'top':
        return {
          pos: new THREE.Vector3(0, Math.max(6, H + 5.5), 0.3),
          target: new THREE.Vector3(0, 0.5, 0),
          fov: 52
        };
      case 'side':
        return {
          pos: new THREE.Vector3(0.6, H * 0.75 + 0.8, W + 3.8),
      ...
      default:
        return {
          pos: new THREE.Vector3(L * 0.85 + 2.5, H + 2.8, W + 2.8),
  ...
  useFrame(() => {
    ...
    if (controls) {
      (controls as OrbitControlsImpl).target.lerp(target, 0.08);
...
export function Scene() {
  ...
  return (
    <Canvas shadows dpr={[1, 1.8]} gl={{ antialias: true, alpha: true, preserveDrawingBuffer: true }} className="h-full w-full touch-none">
      <PerspectiveCamera makeDefault position={[6.8, 4.3, 6.2]} fov={48} />
      <Suspense fallback={null}>
        <Lighting />
        <Environment preset={isInside ? 'apartment' : 'warehouse'} />
        <Truck position={[0, 0, 0]} />
        <PalletManager />
        <EngineeringOverlay />
        <ContactShadows position={[0, -0.02, 0]} opacity={isInside ? 0.22 : 0.42} scale={12} blur={2.5} far={5} />
        <CameraController />
