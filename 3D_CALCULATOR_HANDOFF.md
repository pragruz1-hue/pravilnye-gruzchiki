...
- `<Environment preset="warehouse"/"apartment">` тянет HDR с внешнего CDN `raw.githack.com` — если CDN недоступен у пользователя, `Suspense fallback={null}` = пустая сцена без ошибки. Желательно завендорить HDR локально и уйти с пресетов на `files`.
